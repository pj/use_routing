...
},{"./cjs/react-dom.development.js":"../node_modules/react-dom/cjs/react-dom.development.js"}],"../src/useRouting.tsx":[function(require,module,exports) {
"use strict";
...
exports.matchRouteAndGenerateState = matchRouteAndGenerateState;
...
var VALID_IDENTIFIER = '[a-zA-Z_][a-zA-Z_0-9]*';
var stringType = '(.*)';

function parseRoute(route) {
  var url = new URL(route, 'http://hello');
  var segments = [];
  var params = new Map();
  var typeType = '(string|number|boolean)';
  var typePath = new RegExp("^(".concat(VALID_IDENTIFIER, ")=").concat(typeType, "$"));
  var intType = '(\\d+)';
  var floatType = '(\\d+\.\\d+)';
  var boolType = '(true|false)';
  var stringType = '(.*)';
  var partPath = new RegExp("^(".concat(VALID_IDENTIFIER, ")$"));
  ...
  try {
    for (var _iterator = pathParts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      throw new Error("Invalid path part ".concat(part, " in route ").concat(route));
    } // @ts-ignore
  ...
  try {
    for (var _iterator2 = url.searchParams[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
      ...
      var _match = value.match(typeType);
      ...
      _match = value.match(floatType);

      if (_match) {
        params.set(key, {
          _type: 'number',
          _default: parseFloat(value)
        });
        continue;
      }

      _match = value.match(intType);
      ...
      throw new Error("Invalid query parameter name: ".concat(key, " value: ").concat(value, " in route: ").concat(route));
    }
  } catch (err) {
...
function matchRouteAndGenerateState(hash, routes) {
  hash = hash.replace('#', '');
  var url = new URL(hash, 'https://hello');
  var pathSplit = url.pathname.split('/');
  pathSplit = pathSplit.filter(function (part) {
    return !part.match(/^\s*$/);
  });
  var newState = {};
  ...
  try {
    goto_routes: for (var _iterator3 = routes[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
      var nameRoute = _step3.value;
      var name = nameRoute[0];
      var route = nameRoute[1];
      var routePath = route.path;
      var routeParams = route.params;
      newState = {};

      if (routePath.length === pathSplit.length) {
        for (var n = 0; n < routePath.length; n++) {
          var routePart = routePath[n];
          var urlPart = pathSplit[n];
          ...
          if (routePart._type === 'number') {
            var intParsed = parseInt(urlPart);
            ...
            var floatParsed = parseFloat(urlPart);
          ...
          if (routePart._type === 'string') {
            var match = urlPart.match(stringType);
          ...
          throw new Error("Unable to identify path part ".concat(urlPart, " ").concat(routePart));
        }
        ...
        try {
          for (var _iterator4 = routeParams[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            var param = _step4.value;
            var _name = param[0];
            var options = param[1];
            var paramValue = url.searchParams.get(_name);

            if (paramValue === null && options._default) {
              newState[_name] = options._default;
            }
            ...
            if (options._type === 'boolean') {
              if (paramValue === 'true' || paramValue === 'false') {
                newState[_name] = paramValue === 'true';
              } else {
                throw new Error("Invalid boolean for param: ".concat(_name, " boolean: ").concat(paramValue));
              }
            }

            if (options._type === 'number') {
              var _intParsed = parseInt(paramValue);

              var _floatParsed = parseFloat(paramValue);

              if (!isNaN(_intParsed)) {
                newState[_name] = _intParsed;
              } else if (!isNaN(_floatParsed)) {
                newState[_name] = _floatParsed;
              } else {
                throw new Error("Invalid number for param: ".concat(_name, " number ").concat(paramValue));
              }
            }
          }
        } catch (err) {
  ...
  throw new Error("No valid route found for url: ".concat(hash));
}

function formatUrl(name, params, routes) {
  params = params || {};
  ...
  if (route) {
    ...
    try {
      for (var _iterator5 = route.path[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
        var routePart = _step5.value;
        ...
        var value = params[routePart.name];

        if (!value) {
          throw new Error("Path param ".concat(routePart.name, " not found while generating url for ").concat(name));
        }
    ...
    try {
      for (var _iterator6 = route.params.entries()[Symbol.iterator](), _step6; !(_iteratorNormalCompletion6 = (_step6 = _iterator6.next()).done); _iteratorNormalCompletion6 = true) {
        ...
        if (paramValue) {
          ...
          _value = details._default;
        }

        if (_value) {
          formattedParams.push("".concat(paramName, "=").concat(_value));
          continue;
        }

        throw new Error("Query param ".concat(paramName, " not found and no default while generating url for ").concat(name));
      }
    } catch (err) {
    ...
    if (formattedParams.length > 0) {
      return "/".concat(pathParts.join('/'), "?").concat(formattedParams.join('&'));
    }

    return "/".concat(pathParts.join('/'));
  } else {
    throw new Error("No route found for name ".concat(name));
  }
}

var RoutingContext = React.createContext(null);
var parsedRoutes = new Map();

function useRouter(component, routes) {
  var _React$useState = React.useState(null),
      _React$useState2 = _slicedToArray(_React$useState, 2),
      currentRoute = _React$useState2[0],
      setCurrentRoute = _React$useState2[1];

  Object.entries(routes).forEach(function (nameRoute) {
  ...
  if (currentRoute) {
    routingValue = {
      ...
      navigate: function navigate(name, params) {
        var url = formatUrl(name, params, parsedRoutes);
  ...
  React.useEffect(function () {
    function hashChange(event) {
      ...
      var route = matchRouteAndGenerateState(window.location.hash, parsedRoutes);
...
function useRouting() {
  var currentRoute = React.useContext(RoutingContext);
  return currentRoute;
}
...
var _useRouting = require("../src/useRouting");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

function SubApp(props) {
  var currentRoute = (0, _useRouting.useRouting)();
  ...
  return _react.default.createElement("div", null, _react.default.createElement("div", {
    id: "path"
  }, currentRoute.name), _react.default.createElement("div", {
    id: "params"
  }, JSON.stringify(currentRoute.state, null, 2)), _react.default.createElement("button", {
    id: "back",
    onClick: function onClick(e) {
      return currentRoute.back();
    }
  }, "Back"), _react.default.createElement("button", {
    id: "root",
    onClick: function onClick(e) {
      return currentRoute.navigate('root');
    }
  }, "root"), _react.default.createElement("button", {
    id: "thing",
    onClick: function onClick(e) {
      return currentRoute.navigate('thing');
    }
  }, "thing"), _react.default.createElement("button", {
    id: "other",
    onClick: function onClick(e) {
      return currentRoute.navigate('other', {
        'other_id': 123
      });
    }
    ...
    id: "yet_another",
    onClick: function onClick(e) {
      return currentRoute.navigate('yet_another');
    }
  }, "yet_another"), _react.default.createElement("button", {
    id: "hello_world",
    onClick: function onClick(e) {
      return currentRoute.navigate('hello_world');
    }
  }, "hello_world"), _react.default.createElement("button", {
    id: "blah",
    onClick: function onClick(e) {
      return currentRoute.navigate('blah', {
        'blah_id': 'xxxxx',
        'qwer': 42
      });
    }
  }, "blah"), _react.default.createElement("button", {
    id: "forward",
    onClick: function onClick(e) {
      return currentRoute.forward();
    }
  }, "Forward"));
}

function App(props) {
  return (0, _useRouting.useRouter)(_react.default.createElement(SubApp, null), {
    root: '',
    thing: '/thing',
...
},{"react":"../node_modules/react/index.js","react-dom":"../node_modules/react-dom/index.js","../src/useRouting":"../src/useRouting.tsx"}],"../node_modules/parcel/src/builtins/hmr-runtime.js":[function(require,module,exports) {
